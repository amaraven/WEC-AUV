...
% - wec: WEC object
% - auvFleet: (1xm) cell array of m AUV objects
...
for i = 1:length(simTime)  
    ...
    eStorageBatteryLvl(i+1) = min(eStorageBatteryLvl(i) + (extraPower*energyStorage.n_wecPwrTrnsfr*energyStorage.n_battery + - energyStorage.hotelLoad/energyStorage.n_battery/energyStorage.n_powerTransfer - sum(cellfun(@(auv) auv.wecBatteryDraw, auvFleet))/energyStorage.n_battery )*dt, energyStorage.maxBattery);  % [prev.battery + (draws & gains)*dt]
    ...
    if eStorageBatteryLvl(i+1) < energyStorage.lowBatteryLvl
        ...
        eStorageBatteryLvl(i+1) =  min(eStorageBatteryLvl(i) + (extraPower*energyStorage.n_wecPwrTrnsfr*energyStorage.n_battery + - energyStorage.hotelLoad/energyStorage.n_battery/energyStorage.n_powerTransfer - sum(cellfun(@(auv) (auv.opState(1) == 3 || auv.opState(1) == 2) * auv.hotelLoad / auv.n_powerTransfer / auv.n_battery, auvFleet))/energyStorage.n_battery )*dt, energyStorage.maxBattery);  % [prev.battery + (draws & gains)*dt]
    end
    ...
    cellfun(@(auv) auv.calcBatteryLvl(simTime(i)), auvFleet); 
    auvBatteryLvl(i,:) = cellfun(@(auv) auv.battery(2), auvFleet); 
    ...
    if eStorageLowBatteryFlag == 1
        indxCharging = cellfun(@(auv) auv.opState(1), auvFleet) == 2; 
        cellfun(@(auv) auv.changeOpState(3, simTime(i)), auvFleet(indxCharging));

    else  % start charging auv's docked and still below max battery
        indxNeedCharge = logical( (cellfun(@(auv) auv.opState(1), auvFleet) == 3) .* cellfun(@(auv) auv.battery(2) < auv.maxBattery, auvFleet) );  
        cellfun(@(auv) auv.changeOpState(2, simTime(i)), auvFleet(indxNeedCharge)); 

    end

    % Deployed AUV completes mission & returns
    onMissionLogic = (cellfun(@(auv) auv.opState(1), auvFleet) == 1);
    indxMissionComplete = logical( onMissionLogic .* (cellfun(@(auv) auv.opTimeComplete, auvFleet) <= simTime(i)) );
    cellfun(@(auv) auv.changeOpState(2, simTime(i)), auvFleet(indxMissionComplete));

    % Charging AUV finishes charging: 
    indxChargingComplete = logical( (cellfun(@(auv) auv.opState(1), auvFleet) == 2) .* (cellfun(@(auv) auv.opTimeComplete, auvFleet) <= simTime(i)) ); 
    cellfun(@(auv) auv.changeOpState(3, simTime(i)), auvFleet(indxChargingComplete)); 
    
    if incorpStagger == 1
        staggerHours = cellfun(@(auv) (auv.missionSpecs(2)+auv.chargeTime), auvFleet(1))/length(auvFleet);  % Proportional stagger yileds even breaks in deployment
        if i == 1 
    ...
    auvReadyToDeploy = (cellfun(@(auv) auv.opState(1), auvFleet) ~= 1) .* cellfun(@(auv) auv.battery(2), auvFleet) >= cellfun(@(auv) (auv.rechargeThreshold + auv.missionSpecs(auv.mission, 3)) * auv.maxBattery, auvFleet);  
    ...
    % if any(auvReadyToDeploy ~= 0) 
        ...
        if (eStorageFutureBattery > 0) && (eStorageNoBatteryFlag == 0) 
            cellfun(@(auv) auv.changeOpState(1,simTime(i)), auvFleet(auvToDeploy));
    ...
    auvSchedule(i, :) = cellfun(@(auv) auv.opState(1), auvFleet);
